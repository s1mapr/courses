...
public class StudentController {
  ...
  public String getAllCourses(
      Model model, @ModelAttribute("filter") Filter filter, HttpSession session) {
    ...
    List<CourseDTO> courseDTO =
        courseFilter.doFilter().stream()
            .map(course -> courseService.getCourseData(course.getId()))
            .toList();
    ;
  ...
  public String getCourseById(@PathVariable("id") Long id, Model model, HttpSession session) {
    ...
    if (wasBought) {
      UserCourseMap userCourseMap =
          userCourseMapService.getUserCourseByCourseAndUser(course.getCourse(), user);
  ...
  public String buyCourse(@PathVariable("id") Long id, Model model, HttpSession session) {

    return "student/buyCourse";
  }

  @PostMapping("/buyCourse")
  public String buyCoursePostMethod(HttpServletRequest request, HttpSession session) {
  ...
  public String getBoughtCourses(
      HttpSession session, Model model, @ModelAttribute("filter") Filter filter) {
    User student = (User) session.getAttribute("user");
    model.addAttribute("user", student);
    List<UserCourseMap> userCourseMaps =
        userCourseMapService.getListOfUserCourseMapsByUserForStudent(student);
    List<CourseDTO> courses =
        userCourseMaps.stream()
  ...
  public String getCourseMaterial(
    ...
    CourseMaterial courseMaterial =
        courseMaterialService.getCourseMaterialByCourseMaterialId(materialId);
    model.addAttribute("material", courseMaterial);
    UserCourseMaterialMap userCourseMaterialMap =
        userMaterialMapService.getUserCourseMaterialMapByUserAndMaterial(student, courseMaterial);
  ...
  public String checkAnswers(
    ...
    UserCourseMaterialMap userCourseMaterialMap =
        userMaterialMapService.getUserCourseMaterialMapByUserAndMaterial(student, material);
    materialCourseService.changeMaterialProgressAndStatusIfNeeded(
        student, material, userCourseMaterialMap.getStatus());
  ...
  public String getMaterialTests(
      @PathVariable("courseId") Long courseId,
      @PathVariable("materialId") Long materialId,
      Model model,
      HttpSession session) {
  ...
  public String studentProfile(
      @PathVariable("userId") Long userId, HttpSession session, Model model) {
    User user = (User) session.getAttribute("user");
    model.addAttribute("user", user);
  ...
  public void downloadPdf(HttpServletResponse response, @PathVariable Long id, HttpSession session)
      throws DocumentException, IOException {
    Course course = courseService.getCourseById(id);
    User user = (User) session.getAttribute("user");
    UserCourseMap userCourseMap = userCourseMapService.getUserCourseByCourseAndUser(course, user);
    response.setContentType("application/pdf");
    response.setHeader("Content-Disposition", "attachment; filename=example.pdf");
    ByteArrayOutputStream baos = PdfGenerator.generatePdf(course, user, userCourseMap.getDate());
    response.getOutputStream().write(baos.toByteArray());
    response.getOutputStream().flush();
  }

  @GetMapping("/finishCourse/{courseId}")
  public String finishCourse(HttpSession session, @PathVariable("courseId") Long courseId) {
    User student = (User) session.getAttribute("user");
    Course course = courseService.getCourseById(courseId);
    UserCourseMap userCourseMap =
        userCourseMapService.getUserCourseByCourseAndUser(course, student);
    userCourseMap.setStatus(true);
    userCourseMap.setDate(LocalDate.now());
    userCourseMapService.saveUserCourseMap(userCourseMap);
